...
**进行中**：
- 🚧 模块 002（数据导入）——三文档已批准（v1.0）；纯 Dart 核心切片**已完成**（模型 + WeChatParser CSV/TXT/HTML 流式 + InstagramParser + 预处理 + 导入编排 + Apple 时间转换 + Riverpod 导入状态层，测试 58/58、analyze 0 警告，已过代理评审并修复 SPEC 合规缺口）；iMessage/照片 EXIF 解析器、原生存储、导入 UI 分阶段推进
