...
- `source.segmentationResolved == false` **当且仅当**方向/组成不可判定（会话非空，两分支门控不同，见 §3.1）：**方向分支** = `personSenderIds` 且 `myIdentifiers` 皆未传 + `isFromMe` **全为 `false`**；**多方分支** = 仅 `personSenderIds` 未传 + 目标发送者 >1（`myIdentifiers` 不抑制）。触发时各层与 `identity.confidence` 均为 `Confidence.low`。空会话与"全 `isFromMe==true`（无目标人物消息）"**不**触发（`segmentationResolved==true`）。
...
| 方向不可判定 | 会话非空、`isFromMe` **全为 false**，且 `personSenderIds` **且** `myIdentifiers` **皆未传** | 守卫：不臆断并入；各层与 identity 均 `low`、`source.segmentationResolved==false`；不抛异常（传 `myIdentifiers` 即可定"我"，不触发）|
| 多方会话（群聊）| 解析出 >1 目标发送者，且**仅** `personSenderIds` 未传（`myIdentifiers` 不抑制）| v1 不做多人格拆分：同上守卫（`segmentationResolved==false`、全 `low`），提示用户显式指定后重建 |
...
build(conversation):
  1. splitBySender → (personMessages, userMessages, resolved)
     ...
     守卫（分支门控不同）：方向分支=personSenderIds 且 myIdentifiers 皆未传
     + 会话非空 + isFromMe 全为 false；多方分支=仅 personSenderIds 未传
     + >1 目标发送者（myIdentifiers 不抑制）→ resolved=false（最终强制各层/
     identity 置信度 low、source.segmentationResolved=false）；
     空/无目标场景不触发（resolved=true）
...
| `_indeterminateDirectionConversation()` | 所有消息 `isFromMe==false`（parser 未判方向），验证方向分支守卫降级 |
| `_multiPartyConversation()` | 我方 + >1 个非我方发送者（群聊），验证多方分支守卫（`myIdentifiers` 不抑制）|
...
| 2026-08-02 | v1.0.4（草稿）| PR #10 Owner 复审修订：(🟡) **拆分守卫门控**——区分两分支：**方向不可判定分支**（`isFromMe` 全为 `false`）门控 = `personSenderIds` **且** `myIdentifiers` 皆未传；**多方会话分支**（目标发送者 >1）门控 = **仅** `personSenderIds` 未传（`myIdentifiers` 不抑制）。§1.4 后置条件、§3.1 边界（拆两行）、§4.1 步骤 1 措辞统一；§6.1 增 2 用例（"仅传 `myIdentifiers` 不抑制多方守卫" 断言 `isFalse`、"显式 `personSenderIds` 后正常切分" 断言 `isTrue`）+ §6.2 `_multiPartyConversation()` 夹具 | Claude |

---

> 本文档为**已批准**（2026-08-02），三文档齐全，开发状态**可开发**。所有测试数据仅限合成，严禁使用真实导出数据。
